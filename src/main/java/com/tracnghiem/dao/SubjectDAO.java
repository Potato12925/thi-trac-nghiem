...
public class SubjectDAO extends GenericDAO<Subject> {
	...
	public List<Subject> findPage(int page, int pageSize) {
		...
		return getSession().createQuery(hql, Subject.class).setFirstResult(offset).setMaxResults(pageSize).list();
	}
	...
	public boolean existsByName(String name) {
		String hql = "SELECT COUNT(*) FROM Subject s WHERE s.deleted = false AND lower(s.subjectName) = :name";
		long count = getSession().createQuery(hql, Long.class).setParameter("name", name.trim().toLowerCase())
				.uniqueResult();
		return count > 0;
	}
}
