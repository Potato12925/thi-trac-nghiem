...
public class StudentService {
	...
	public List<UpcomingExamDTO> getUpcomingExams(Student student, Date now) {
		...
		List<LecturerRegistration> registrations = lecturerRegistrationDAO.findUpcomingExamsForStudent(studentId,
				classId, now);
		return registrations.stream().map(this::toUpcomingExamDTO).collect(Collectors.toList());
	}
	...
	private UpcomingExamDTO toUpcomingExamDTO(LecturerRegistration registration) {
		...
		dto.setSubjectId(
				registration.getSubject() == null ? null : normalize(registration.getSubject().getSubjectId()));
		dto.setSubjectName(
				registration.getSubject() == null ? null : normalize(registration.getSubject().getSubjectName()));
	...
	public void savePendingActions(String actionsData) {
		...
		for (String line : lines) {
			...
			String classId = parts.length > 7 ? parts[7].trim() : "";

			actions.add(new StudentActionDTO(type, studentId, lastName, firstName, birthDate, address, email, classId));
		}
		...
		for (StudentDTO dto : dtos) {
			...
			if (classroomService.findClassroomById(classId) == null) {
				throw new IllegalArgumentException(
						"Mã lớp '" + classId + "' không tồn tại trong hệ thống (Mã SV: " + studentId + ").");
			}
