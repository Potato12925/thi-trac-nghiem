...
/**
 * Service layer for Subject (MonHoc) management. Handles business logic,
 * validation, CRUD operations, and undo functionality.
 * 
...
public class SubjectService {
	...
	/**
	 * Validates subject code (maMH) Rules: - Not null or empty - Exactly 5
	 * characters - Contains only alphanumeric characters
	 * 
	...
	/**
	 * Validates subject name (tenMH) Rules: - Not null or empty - Maximum 40
	 * characters
	 * 
	...
	/**
	 * Checks if subject name already exists in database (excluding specified
	 * subject)
	 * 
	 * @param tenMH       Subject name
	 ...
	 * Tables that reference MonHoc: - GiaoVienDangKy (Subject registration) - BoDe
	 * (Question bank) - BaiThi (Exam) - BangDiem (Grade)
	 * 
	 * @param maMH Subject code
	...
	public String validateDeleteConstraint(String maMH) {
		...
		if (teacherRegistrationCount > 0) {
			return "Không thể xóa môn học. Có " + teacherRegistrationCount + " giáo viên đã đăng ký môn học này";
		}
	 ...
	 * 
	 * Process: 1. Validate maMH 2. Validate tenMH 3. Check if maMH already exists
	 * 4. Check if tenMH already exists 5. Create subject 6. Save operation to
	 * history for undo
	 * 
	 * @param maMH  Subject code
	 * @param tenMH Subject name
	 ...
	 * 
	 * Process: 1. Check if subject exists 2. Validate tenMH 3. Check if new tenMH
	 * already exists (excluding current subject) 4. Update subject 5. Save
	 * operation to history for undo
	 * 
	 ...
	 * 
	 * Process: 1. Validate if subject exists 2. Check foreign key constraints 3.
	 * Delete subject 4. Save operation to history for undo
	 * 
	 * @param maMH Subject code
	 * @return Error message if validation fails or constraints violated, null if
	 *         success
	 */
	@Transactional
	...
	/**
	 * Searches subjects by keyword. Searches in both maMH and tenMH fields.
	 * 
	...
	/**
	 * Performs undo operation for the last action. Supported undo operations:
	 * CREATE, UPDATE, DELETE
	 * 
	 * Process: 1. Pop last operation from stack 2. Reverse the operation: - CREATE
	 * -> DELETE - UPDATE -> UPDATE to old values - DELETE -> CREATE 3. Apply
	 * reversed operation to database
	 * 
	...
	/**
	 * Clears operation history. Used for session cleanup.
	 */
	...
	/**
	 * Represents an operation for undo/redo functionality. Stores: - Type of
	 * operation (CREATE, UPDATE, DELETE) - Entity data after operation - Old entity
	 * data (for UPDATE)
	 */
