...
<c:set var="lecturerName"
	value="${empty lecturerProfile ? sessionScope.LOGIN_USER : lecturerProfile.lastName}
              ${empty lecturerProfile ? '' : lecturerProfile.firstName}" />

<c:set var="lecturerId"
	value="${empty lecturerProfile.lecturerId ? sessionScope.LOGIN_USER : lecturerProfile.lecturerId}" />

<c:set var="totalQuestionsValue"
	value="${empty totalQuestions ? 0 : totalQuestions}" />
<c:set var="totalLecturerRegistrationsValue"
	value="${empty totalLecturerRegistrations ? 0 : totalLecturerRegistrations}" />
<c:set var="totalScoresPublishedValue"
	value="${empty totalScoresPublished ? 0 : totalScoresPublished}" />
<c:set var="lastActiveSubjectValue"
	value="${empty lastActiveSubject ? 'Chua co du lieu' : lastActiveSubject}" />
		...
		<div class="profile-top">
			<div class="avatar-box">
				<c:choose>
					<c:when test="${not empty lecturerProfile.firstName}">
					    ${fn:substring(lecturerProfile.firstName,0,1)}
					</c:when>
			...
			<div>
				...
				<div class="Lecturer-meta">
					<i class="bi bi-person-badge"></i> Ma giang vien: ${lecturerId}
				</div>
				<div class="Lecturer-meta">
					<i class="bi bi-briefcase"></i> Vai tro: Giang vien
				</div>
				<div class="Lecturer-meta">
					<i class="bi bi-calendar-event"></i> Hom nay:
